...
\pause Usando `arrange` y `mutate` indique cuanto es la diferencia entre las personas con la edad menor. 

\pause Combinando `filter` y `mutate` indique cuantas personas hay casadas.

\pause Repita lo anterior, pero con la cantidad de personas que tienen sexo `'M'`
...
\pause
¿Qué provincia tiene más personas aseguradas? Recuerde que `TRUE == 1` y que `FALSE == 0`. \pause ¿Cual condición de aseguramiento tiene la mayor diversidad de provincias? Puede apoyarse en la función `n_distinct`.
