...
>- Componentes individuales: Año = `2017`, Mes = `11`, Día = `1`
...
Hijos <- Hijos %>% 
  mutate(Nacimiento_Hijo = ymd(Nacimiento_Hijo))
...
Fallecimientos <- Fallecimientos %>% 
  transmute(Fecha_tiempo = make_datetime(year = Anno_Muerte,
                           month = Mes_Muerte,
                           day = Dia_Muerte,
                           hour = Hora_Muerte,
                           min = Minuto_Muerte),
...
```{r}
fecha_g <- ymd_hms('2017-10-20 15:35:20',
                   tz = 'America/Guatemala')
fecha_g_2 <- ymd_hm('2017-10-20 15:35',
                   tz = 'America/Guatemala')
fecha_g_3 <- ymd_h('2017-10-20 15',
                   tz = 'America/Guatemala')
```
...
\pause Las funciones `month`.`mday` tiene dos parámetros opcionales

\pause
Compruebe el valor en `fecha_g` de las funciones anteriores.


## Ejercicio:

Haga un gráfico de barras (en ggplot), usando el día de la semana de las fechas de nacimiento. 
...
```{r}
tiempo_lectivo <- today() - ymd(20170807)
...
¿Qué pasa si nos devolvemos un año desde el primero de enero? 
\pause
...
```{r}
fecha_falla <- ymd_hms('20170312 00:00:00',tz = 'America/New_York')
fecha_falla + ddays(1)
```
...
```{r}
marzo31 <- ymd(20170331)  
marzo31 + months(1)
marzo31 + days(31)
marzo31 %m+% months(1) # Para restar se usa %m-%
```

Pero se tiene que tener **mucho** cuidado con el orden, pues no lo preservan...
